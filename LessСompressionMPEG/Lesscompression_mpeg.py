...
def main(anchorFrame, targetFrame, saveOutput=True, blockSize=32):
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
    ...
    if saveOutput:
        ...
        cv2.imwrite(outfile + "anchorFrame.jpg", anchorFrame)
        cv2.imwrite(outfile + "targetFrame.jpg", targetFrame)
        cv2.imwrite(outfile + "diffFrameRGB.jpg", diffFrameRGB)
        cv2.imwrite(outfile + "predictedFrameRGB.jpg", predictedFrameRGB)
        cv2.imwrite(outfile + "residualFrameRGB.jpg", residualFrameRGB)
        cv2.imwrite(outfile + "restoreFrameRGB.jpg", restoreFrameRGB)
        barWidth = 0.3
        ...
        plt.bar(br2, P1, color='r', width=barWidth,
                edgecolor='grey', label='Розмір для початкового кадру')
        plt.bar(br3, Diff, color='g', width=barWidth,
                edgecolor='grey', label='Розмір для різниці між кадрами')
        plt.bar(br4, Mpeg, color='b', width=barWidth,
...
if __name__ == "__main__":
    ...
    main(frame1, frame2, saveOutput=True, blockSize=16)
